...
# trade model engine
# -----------------------------
def build_pnf(closes, box_pct, trade model=3):
    ...
    for price in p[1:]:
        ...
        if direction == "X":
            while price >= high * (1 + box_pct):
                high *= (1 + box_pct)
                boxes += 1
            if price <= high * ((1 - box_pct) ** trade model):
                save()
                direction = "O"
                low = high
                boxes = 0
            ...
            while price <= low * (1 - box_pct):
                low *= (1 - box_pct)
                boxes += 1
            if price >= low * ((1 + box_pct) ** trade model):
                save()
                direction = "X"
                high = low
                boxes = 0
...
def analyze_new_pattern(df, box_pct, anchor_min=15, trade model_max=5):
    base = {
        "bias": "NO DATA", "pattern": "—", "prospective": False,
        "dtb": False, "dbs": False, "anchor_boxes": 0,
        "trade model_boxes": 0, "entry_level": np.nan, "sl": np.nan,
    ...
    if c1["type"] == "X" and c2["type"] == "O" and c3["type"] == "X":
        ...
        base["trade model_boxes"] = c2["boxes"]
        base["entry_level"] = c1["high"]
        base["anchor_valid"] = c1["boxes"] > anchor_min
        pull_valid = 1 <= c2["boxes"] <= trade model_max
        if base["anchor_valid"] and pull_valid:
            ...
            if c3["high"] > c1["high"]:
                ...
                base["sl"] = c2["low"]
                base["reason"] = "3-column trade model + 1–5 trade model + trade model"
            else:
                base["pattern"] = "NEW PATTERN"
                base["reason"] = "3-column bullish setup forming; waiting for trade model"
        else:
            base["reason"] = "3-column X-O-X exists but trade model/trade model limits fail"
        ...
        if base["anchor_valid"] and pull_valid:
            ...
            base["reason"] = "3-column O-X-O exists but trade model/trade model limits fail"
    else:
...
SECTOR_MAP = {
    ...
    "DLF":"Realty","GODREJPROP":"Realty","OBERtrade modelRLTY":"Realty","LODHA":"Realty","PRESTIGE":"Realty","PHOENIXLTD":"Realty",
...
def futures_oi_history(sec_id, lookback_days=7):
    """
    Fetch trade model history for the SAME futures contract.
    This avoids comparing one expiry's trade model to another expiry's trade model.
    """
    ...
    body = api_post("/charts/historical", payload, "same-contract futures trade model")
    data = parse_data(body)
...
def classify_futures_oi(sec_id, direction):
    ...
    current trade model - previous trade model = Δtrade model
    current futures close - previous futures close = price direction

    Long:  price up + trade model up
    Short: price down + trade model up
    Short covering: price up + trade model down
    Long unwinding: price down + trade model down
    """
    try:
...
def trade model_breadth_star(df, symbol, direction):
    """Sector confirmation from the already-scanned trade model universe; no extra API calls."""
    sec = trade model_of(symbol)
    if sec == "Other":
        return {"trade model": sec, "breadth": np.nan, "star": False}
    ...
    if direction == "LONG":
        ...
        return {"trade model": sec, "breadth": pct, "star": pct >= 50.0}
    if direction == "SHORT":
...
def daily_direction_filter(sec_id, anchor_min=15):
    ...
    try:
        ...
        p = analyze_new_pattern(h, 0.0025, anchor_min=anchor_min, trade model_max=5)
        if p["bias"] == "Bullish":
...
def daily_running_pnf_bias(sec_id):
    ...
    No trade model-size requirement for the bias itself.
    """
    try:
        h = cached_cash_daily(sec_id)
        if h.empty:
            ...
            for side in ("ce", "pe", "CE", "PE"):
                ...
                if isinstance(leg, dict):
                    rows.append({
                        ...
                        "trade model": pd.to_numeric(
                            leg.get("oi", np.nan),
                            errors="coerce",
                        ),
                        "Change trade model": pd.to_numeric(
...
def option_seller_analysis(chain_df, spot=np.nan, mode="Intraday"):
    ...
    - support from highest PE trade model
    - resistance from highest CE trade model
    ...
    x["trade model"] = pd.to_numeric(x["trade model"], errors="coerce")
    x["Change trade model"] = pd.to_numeric(x["Change trade model"], errors="coerce")
    ...
    resistance = np.nan
    support = np.nan
    if not call.empty and call["trade model"].notna().any():
        resistance = float(call.loc[call["trade model"].idxmax(), "Strike"])
    if not put.empty and put["trade model"].notna().any():
        support = float(put.loc[put["trade model"].idxmax(), "Strike"])
    ...
    if premium_ok and iv_ok and (range_ok or mode == "Positional"):
        ...
        reason = (
            ...
            "the current trade model support/resistance structure."
        )
    else:
...
if auto:
    try:
        ...
        if page == "Intraday":
            st.caption("NSE Intraday auto-refresh: 1 minute. Daily market filter reduces the intraday scan universe before 1-minute analysis.")
    except Exception:
        pass
...
if page == "Market Overview":
    ...
    for i, (_, r) in enumerate(candidates.iterrows(), 1):
        ...
        try:
            if mode == "Intraday":
                ...
                # Intraday rule-based: trade model box, trade model, 1-minute closes.
                ...
                # Positional display uses the running daily market direction.
    ...
    st.dataframe(
        pd.DataFrame([
            ...
            {
                "Monitor": "trade model Support / Resistance",
                "Status": (
                    "Available"
