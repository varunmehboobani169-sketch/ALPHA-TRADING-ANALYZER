...
if page == "Market Overview":
    ...
    if mode == "Positional":
        ...
        st.caption(
            "Intraday: positional bias from 0.25% daily P&F → "
            "0.15% / 3-box / 1-minute P&F → 10-SMA direction filter. "
            "Long only above SMA10; short only below SMA10."
        )
        ...
        st.info(
            f"Positional bias filter retained {len(candidates)} of {len(fut)} NSE F&O stocks. "
            f"Bias is fixed for the trading day."
        )

        if st.button("Rebuild Positional Bias", key="rebuild_positional_bias"):
            st.session_state.intraday_daily_filter = {}
            st.session_state.intraday_filter_date = None
            st.rerun()
