...
TIMEFRAMES = {"ST": 21, "MT": 63, "IT": 126, "LT": 252}
...
def _session() -> requests.Session:
    s = requests.Session()
    s.headers.update({"User-Agent": "Mozilla/5.0 MatrixEngine/1.0", "Referer": "https://www.niftyindices.com/", "Accept": "text/csv,application/octet-stream,text/plain,*/*"})
    return s
...
def load_index_members(index_name: str) -> pd.DataFrame:
    ...
    result = pd.concat(frames, ignore_index=True).drop_duplicates("symbol", keep="first")
    return result[result["symbol"].ne("") & result["symbol"].ne("NAN")].reset_index(drop=True)


def fetch_dhan_instruments() -> pd.DataFrame:
    r = _session().get("https://images.dhan.co/api-data/api-scrip-master.csv", timeout=60)
    r.raise_for_status()
...
def resolve_security_ids(members: pd.DataFrame, instruments: pd.DataFrame) -> pd.DataFrame:
    ...
    if col_seg2:
        # Prefer NSE cash-equity rows. Some revisions use E, NSE_EQ, or EQUITY text.
        seg = x[col_seg2].astype(str).str.upper().str.strip()
        mask &= seg.isin({"E", "NSE_EQ", "EQUITY"}) | seg.str.contains("NSE.*EQ|EQUITY", regex=True, na=False)
    if col_inst:
        inst = x[col_inst].astype(str).str.upper().str.strip()
        eq_mask = inst.eq("EQUITY") | inst.str.contains("EQUITY", na=False)
        if eq_mask.any():
            mask &= eq_mask
    ...
    if "security_id" not in left.columns:
        left["security_id"] = ""

    merged = left.merge(x[["_trading_symbol", "_custom_symbol", "_security_id"]], left_on="symbol", right_on="_trading_symbol", how="left", suffixes=("", "_mapped"))
    mapped = merged["_security_id_mapped"] if "_security_id_mapped" in merged.columns else merged["_security_id"]
    merged["security_id"] = mapped.fillna(merged["security_id"]).astype(str).str.strip()

    if col_custom:
        missing = merged["security_id"].isin({"", "nan", "None"})
        if missing.any():
            ...
            merged.loc[missing, "security_id"] = repl["_security_id"].fillna("").astype(str).values

    return merged.drop(columns=[c for c in ["_trading_symbol", "_custom_symbol", "_security_id_mapped"] if c in merged.columns], errors="ignore")


def fetch_dhan_daily(client_id: str, access_token: str, security_id: str, from_date: str, to_date: str) -> pd.DataFrame:
    r = requests.post("https://api.dhan.co/v2/charts/historical", headers={"Accept": "application/json", "Content-Type": "application/json", "access-token": access_token, "client-id": client_id}, json={"securityId": str(security_id), "exchangeSegment": "NSE_EQ", "instrument": "EQUITY", "expiryCode": 0, "oi": False, "fromDate": from_date, "toDate": to_date}, timeout=REQUEST_TIMEOUT)
    if r.status_code >= 400:
        raise RuntimeError(f"Dhan historical {security_id}: HTTP {r.status_code} {r.text[:300]}")
    body = r.json()
    ts, closes = body.get("timestamp") or body.get("start_Time") or [], body.get("close") or []
    highs, lows = body.get("high") or [], body.get("low") or []
...
def pattern_score(close: pd.Series, n: int) -> int:
    if len(close) < n + 5:
        return 0
    c, prev = float(close.iloc[-1]), float(close.iloc[-n])
...
def score_stock(stock: pd.DataFrame, benchmark: pd.DataFrame) -> Dict[str, object]:
    ...
    for label, n in TIMEFRAMES.items():
        ps, rs = pattern_score(s, n), rs_score(s, b, n)
        row[f"{label}_price"], row[f"{label}_rs"], row[f"{label}_swing_pct"] = ps, rs, swing_pct(s, n)
        price_total += ps
        rs_total += rs
    row["price_score"], row["rs_score"], row["composite"] = price_total, rs_total, price_total + rs_total
    score = int(row["composite"])
    row["grade"] = "A+" if score >= 12 else "A" if score >= 8 else "B" if score >= 4 else "C-" if score <= -12 else "C" if score <= -8 else "B-" if score <= -4 else "NEUTRAL"
...
def build_matrix_rows(members: pd.DataFrame, histories: Dict[str, pd.DataFrame], benchmark: pd.DataFrame, universe_name: str, benchmark_name: str) -> pd.DataFrame:
    ...
    for _, member in members.iterrows():
        ...
        if not sid or sid.lower() in {"nan", "none"} or sid not in histories:
            continue
        result = score_stock(histories[sid], benchmark)
        if not result.get("valid"):
            continue
        result.update({"symbol": member["symbol"], "company": member.get("company", ""), "universe": universe_name, "rs_benchmark": benchmark_name, "source_index": member.get("source_index", ""), "security_id": sid})
        rows.append(result)
    if not rows:
        return pd.DataFrame()
    df = pd.DataFrame(rows).sort_values(["composite", "price_score", "rs_score"], ascending=False, kind="mergesort").reset_index(drop=True)
    df["rank"] = df.index + 1
    return df
...
def universe_counts(df: pd.DataFrame) -> Dict[str, int]:
    ...
    return {"stocks": int(len(df)), "bullish": int((df["composite"] > 0).sum()), "neutral": int((df["composite"] == 0).sum()), "bearish": int((df["composite"] < 0).sum())}
