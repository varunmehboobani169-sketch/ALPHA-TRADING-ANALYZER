...
if page == "Market Overview":
    ...
    ltp_count = sum(
        1 for sid in ids
        if pd.notna(pd.to_numeric(ltp_map.get(int(sid)), errors="coerce"))
    )
    ...
    res = pd.DataFrame(rows)
    display_cols = ["Script", "LTP", "Bias", "Entry", "SL", "Trade Recommendation"]
    ...
    if res.empty:
        ...
        st.dataframe(
            res[display_cols].style.apply(highlight_trade, axis=1),
            use_container_width=True,
            hide_index=True,
        )
