...
def option_quarter_download(q_label, start_date, end_date, token, client_id, progress_cb=None, status_cb=None):
    ...
    jobs = []
    for cur, ce in month_chunks(start_date, end_date, 10):
    ...
    # Preflight: prove Dhan accepts the exact weekly-option request before launching the full download.
    if status_cb:
    ...
    if status_cb:
        status_cb(f"Preflight passed. Downloading {total:,} Dhan requests with {MAX_OPTION_WORKERS} workers...")

    remaining = jobs[1:]
    with ThreadPoolExecutor(max_workers=MAX_OPTION_WORKERS) as executor:
        futures = [executor.submit(run_option_job, job, token, client_id, q_label) for job in remaining]
        for future in as_completed(futures):
            ...
            if status_cb and (done % 10 == 0 or done == total):
                status_cb(f"{done:,}/{total:,} requests complete")
...
def render_data_vault(token, client_id):
    ...
    if dataset == "NIFTY Weekly Options ATM±10":
        st.info("1-minute weekly expired-option data. 21 strike levels × CE/PE. OI + IV + volume + spot. Dhan requests are split into 10-day windows. A preflight request runs first, then 3 parallel workers speed up collection.")
    else:
    ...
    if st.button("DOWNLOAD QUARTER DATA", use_container_width=True):
        ...
        try:
            ...
            for qi, (label, (qs, qe)) in enumerate(ranges_to_get):
                base = qi / max(1, len(ranges_to_get))
                span = 1 / max(1, len(ranges_to_get))
                status.info(f"{label}: starting")

                def update_progress(p, base=base, span=span):
                    bar.progress(min(99, int((base + p * span) * 100)), text=f"FRIDAY Data Vault: {int((base + p * span) * 100)}% — {label}")

                def update_status(msg, label=label):
                    status.info(f"{label}: {msg}")
            ...
            if len(parts) == 1:
                ...
                with zipfile.ZipFile(out, "w", zipfile.ZIP_DEFLATED) as z:
                    ...
                    z.writestr("README.txt", f"FRIDAY research data package. Period={period}. Options are 1-minute weekly ATM-10..ATM+10 with OI/IV/volume/spot. Requests are split into 10-day windows. NIFTY/VIX are selected OHLC timeframe with OI disabled.")
