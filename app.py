...
def synchronize(options, spot, vix):
    ...
    merged = pd.merge_asof(o, s, on="timestamp", direction="backward", tolerance=SPOT_TOLERANCE)
...
def pair_ce_pe(merged):
    """Pair CE and PE at the same strike/expiry with <=2 minutes timestamp difference."""
    if merged.empty:
        return pd.DataFrame()
    ...
    # Rename every PE field that also exists on CE before merge.
...
def build_features(merged):
    ...
    f = pd.DataFrame({
        ...
        "ce_close": series_for("close"),
        "pe_close": series_for("close_pe"),
        "ce_iv": series_for("iv"),
        "pe_iv": series_for("iv_pe"),
        "ce_oi": series_for("oi"),
        "pe_oi": series_for("oi_pe"),
        "ce_volume": series_for("volume"),
        "pe_volume": series_for("volume_pe"),
        "vix_close": series_for("vix_close_pair"),
        "pair_gap_seconds": series_for("pair_gap_seconds"),
    })
...
with st.sidebar:
    ...
    if access_token:
        st.success("Dhan token entered for this session")
...
if not opt_files or not spot_files:
    ...
    if st.button("ANALYZE PATTERNS", use_container_width=True):
        ...
        try:
            ...
            o_raw = read_csvs(opt_files)
            s_raw = read_csvs(spot_files)
            v_raw = read_csvs(vix_files) if vix_files else pd.DataFrame()
            ...
            status.info("65% — Pairing CE and PE")
            bar.progress(65, text="FRIDAY: 65% — Pairing CE/PE within ±2 minutes")
            paired = pair_ce_pe(m)
            if paired.empty:
                raise ValueError("No CE/PE pairs found within ±2 minutes for the same strike and expiry. Check option timestamps, strike, expiry and CE/PE labels.")

            status.info("75% — Selecting ATM")
            bar.progress(75, text="FRIDAY: 75% — Selecting ATM")
            f = build_features(m)
            if f.empty:
                raise ValueError("CE/PE pairing worked, but no ATM observations could be built.")
            ...
            st.dataframe(diagnostics(o, s, v, m, paired, f), use_container_width=True, hide_index=True)
            ...
            with zipfile.ZipFile(out, "w", zipfile.ZIP_DEFLATED) as z:
                ...
                z.writestr("FRIDAY_diagnostics.csv", diagnostics(o, s, v, m, paired, f).to_csv(index=False))
                z.writestr("FRIDAY_pairs.csv", paired.to_csv(index=False))
