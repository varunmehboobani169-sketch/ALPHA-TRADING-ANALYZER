...
def normalize_spot(df):
    x = normalize_time(df); p = num_col(x, ["close", "ltp", "last_price", "nifty", "spot", "index_close", "price"])
...
def normalize_vix(df):
    x = normalize_time(df); p = num_col(x, ["close", "ltp", "last_price", "vix_close", "vix", "price"])
...
def pair_ce_pe(m):
    ...
    paired["pair_timestamp"] = paired.timestamp; paired["nifty_spot_pair"] = paired.nifty_spot; paired["vix_close_pair"] = paired.get("vix_close", np.nan); paired["pair_gap_seconds"] = (paired.pe_timestamp - paired.timestamp).abs().dt.total_seconds()
...
def build_features(m):
    ...
    f["pcr_oi"] = f.pe_oi / f.ce_oi.replace(0, np.nan); f["straddle"] = f.ce_close + f.pe_close; f["atm_iv"] = pd.concat([f.ce_iv, f.pe_iv], axis=1).mean(axis=1); f = f.sort_values("timestamp").reset_index(drop=True)
    ...
    for n in [4, 16]: f[f"forward_spot_{n}"] = f.nifty_spot.shift(-n) / f.nifty_spot - 1; f[f"forward_straddle_{n}"] = f.straddle.shift(-n) / f.straddle - 1
    return f
...
def dhan_call(path, payload, token, client_id):
    ...
    except urllib.error.URLError as e: raise RuntimeError(f"Dhan connection error: {e}") from e
...
def render_data_vault(token,client_id):
    ...
    if st.button("DOWNLOAD OHLC",use_container_width=True):
        ...
        try:
            status.info("20% — Preparing Dhan instrument request"); bar.progress(20,text="FRIDAY Data Vault: 20%")
            df,chosen=download_index_ohlc("NIFTY" if dataset=="NIFTY" else "INDIA VIX",start_date,end_date,timeframe,token,client_id)
            status.info(f"70% — Data received ({chosen[0]}/{chosen[1]})"); bar.progress(70,text="FRIDAY Data Vault: 70% — Data received")
            df["timestamp"]=df.timestamp.dt.strftime("%Y-%m-%d %H:%M:%S"); csv=df.to_csv(index=False).encode()
        ...
        except Exception as e: status.error(f"Download stopped: {e}"); st.exception(e)


def render_analyzer():
    st.title("FRIDAY — OPTION PATTERN RESEARCH"); st.caption("Options + NIFTY Spot + India VIX only. Futures are excluded.")
    c1,c2,c3=st.columns(3)
    with c1: period=st.selectbox("Research Period",["Q1 2024","Q2 2024","Q3 2024","Q4 2024","Full 2024","Custom"],index=0)
    with c2: custom_start=st.date_input("Custom start",value=date(2024,1,1))
    with c3: custom_end=st.date_input("Custom end",value=date(2024,3,31))
    ranges={"Q1 2024":(date(2024,1,1),date(2024,3,31)),"Q2 2024":(date(2024,4,1),date(2024,6,30)),"Q3 2024":(date(2024,7,1),date(2024,9,30)),"Q4 2024":(date(2024,10,1),date(2024,12,31)),"Full 2024":(date(2024,1,1),date(2024,12,31)),"Custom":(custom_start,custom_end)}
    start_sel,end_sel=ranges[period]
    st.caption(f"Selected period: {start_sel} → {end_sel}")
    opt=st.file_uploader(f"{period} Option Data (CSV)",type=["csv"],accept_multiple_files=True); spot=st.file_uploader(f"{period} NIFTY Spot Data (CSV)",type=["csv"],accept_multiple_files=True); vix=st.file_uploader(f"{period} India VIX Data (CSV)",type=["csv"],accept_multiple_files=True)
    if not opt or not spot: st.info("Upload Option and NIFTY Spot CSV files. India VIX is optional."); return
    if st.button("ANALYZE PATTERNS",use_container_width=True):
        bar=st.progress(0,text="FRIDAY: 0%"); status=st.empty()
        try:
            bar.progress(10,text="FRIDAY: 10% — Reading files"); o=normalize_options(read_csvs(opt)); s=normalize_spot(read_csvs(spot)); v=normalize_vix(read_csvs(vix)) if vix else pd.DataFrame()
            ...
            report_text,safe=build_report(period,o,s,v,m,p,f,patterns)
            st.subheader(f"FRIDAY — OPTION PATTERN RESEARCH — {period}")
            st.subheader("Input Diagnostics"); st.dataframe(diagnostics(o,s,v,m,p,f),use_container_width=True,hide_index=True); st.subheader("Pattern Summary"); st.dataframe(patterns,use_container_width=True,hide_index=True); st.subheader("Feature Data"); st.dataframe(f.tail(500),use_container_width=True,hide_index=True)
            st.download_button("DOWNLOAD REPORT (.MD)",report_text.encode("utf-8"),f"FRIDAY_{safe}_REPORT.md","text/markdown",use_container_width=True)
            out=io.BytesIO()
            with zipfile.ZipFile(out,"w",zipfile.ZIP_DEFLATED) as z:
                z.writestr(f"FRIDAY_{safe}_REPORT.md",report_text); z.writestr(f"FRIDAY_{safe}_features.csv",f.to_csv(index=False)); z.writestr(f"FRIDAY_{safe}_patterns.csv",patterns.to_csv(index=False)); z.writestr(f"FRIDAY_{safe}_diagnostics.csv",diagnostics(o,s,v,m,p,f).to_csv(index=False)); z.writestr(f"FRIDAY_{safe}_pairs.csv",p.to_csv(index=False))
            st.download_button("DOWNLOAD FULL RESEARCH PACKAGE (.ZIP)",out.getvalue(),f"FRIDAY_{safe}_RESEARCH.zip","application/zip",use_container_width=True)
        except Exception as e: status.error(f"Processing stopped: {e}"); st.exception(e)

with st.sidebar:
    st.subheader("FRIDAY"); client_id=st.text_input("Dhan Client ID",value=DEFAULT_CLIENT_ID).strip(); token=st.text_input("Dhan Access Token",value="",type="password").strip()
