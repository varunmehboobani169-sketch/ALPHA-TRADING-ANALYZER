...
def normalize_options(df):
    ...
    if expiry is not None:
        ...
        out["expiry"] = pd.Series(pd.NaT, index=df.index, dtype="datetime64[ns]")
    for names, target in [
...
def pair_ce_pe(aligned, has_expiry):
    ...
    if out.empty:
        return out
    out["pair_gap_seconds"] = (pd.to_datetime(out["timestamp"]) - pd.to_datetime(out["pe_timestamp"])).dt.total_seconds().abs()
...
def build_market_features(pairs, spot, vix):
    ...
    p["dte"] = np.nan
    if "expiry" in p.columns:
        valid = p["expiry"].notna() & p["timestamp"].notna()
        p.loc[valid, "dte"] = (p.loc[valid, "expiry"] - p.loc[valid, "timestamp"].dt.normalize()).dt.total_seconds() / 86400.0
    if p["expiry"].notna().any() if "expiry" in p.columns else False:
    ...
    keep = ["timestamp", "expiry", "strike", "spot", "vix", "close", "pe_close", "iv", "pe_iv", "oi", "pe_oi", "volume", "pe_volume", "pair_gap_seconds", "dte"]
    ...
    if not v.empty:
        ...
        if "vix_state" in f.columns:
            f["vix"] = f["vix_state"].combine_first(f.get("vix", np.nan))
            f = f.drop(columns=["vix_state"])
    for c in ["ce_close", "pe_close", "straddle", "ce_iv", "pe_iv", "iv_mid", "pcr_oi", "oi_imbalance", "volume_imbalance"]:
        f[f"{c}_chg_1"] = pd.to_numeric(f[c], errors="coerce").pct_change(1)
        f[f"{c}_chg_5"] = pd.to_numeric(f[c], errors="coerce").pct_change(5)
        f[f"{c}_chg_15"] = pd.to_numeric(f[c], errors="coerce").pct_change(15)
    ...
    if "expiry" in f.columns:
        ...
        f["is_expiry_day"] = False
    f["straddle_level_log"] = np.log(pd.to_numeric(f["straddle"], errors="coerce").clip(lower=1e-6))
...
def forward_values(base, target, source_col, horizon_min, tolerance):
    ...
    left["target_ts"] = pd.to_datetime(left["timestamp"]) + pd.Timedelta(minutes=horizon_min)
...
def add_forward_outcomes(f, spot):
    ...
    for h in HORIZONS_MIN:
        ...
        x[f"future_straddle_ret_{h}m"] = pd.to_numeric(fut_str, errors="coerce").to_numpy() / pd.to_numeric(x["straddle"], errors="coerce").to_numpy() - 1
        x[f"future_spot_ret_{h}m"] = pd.to_numeric(fut_spot, errors="coerce").to_numpy() / pd.to_numeric(x["spot"], errors="coerce").to_numpy() - 1
    return x
...
def build_candidate_rules(df):
    ...
    if target_col not in df.columns:
        return pd.DataFrame()
    # Exclude booleans explicitly. Pandas/NumPy quantile computation on bool
    # can attempt boolean subtraction and crash the whole research run.
    numeric = [
        c for c in df.columns
        if pd.api.types.is_numeric_dtype(df[c])
        and not pd.api.types.is_bool_dtype(df[c])
        and c not in {"atm_strike", "time_minute"}
    ]
...
def build_interactions(df, top_features):
    ...
    for i in range(len(cols)):
        for j in range(i + 1, len(cols)):
            ...
            if pd.api.types.is_bool_dtype(df[a]) or pd.api.types.is_bool_dtype(df[b]):
                continue
...
def process_quarter(q, options_all, spot_all, vix_all):
    ...
    if pairs.empty:
        raise ValueError(f"{q}: no valid CE/PE pairs after pairing.")
    ...
    warnings = []
    if not has_expiry:
        warnings.append("Options file has no historical expiry field; FRIDAY used rolling-contract same-strike pairing and did not infer expiry from a current calendar.")
    if fruit.empty:
        warnings.append("No single-variable candidate met the minimum quality threshold.")
    ...
    if st.button("RUN FRIDAY DEEP AUDIT", use_container_width=True):
        ...
        try:
            ...
            for i, q in enumerate(selected, start=1):
                ...
                try:
                    results[q] = process_quarter(q, options_all, spot_all, vix_all)
                except Exception as exc:
