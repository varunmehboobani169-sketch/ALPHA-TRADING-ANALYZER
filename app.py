...
if page == "Fresh Trades":
    ...
    # Add the ORIGINAL trade-trigger timestamp to the positional scanner table.
    # This is taken from the immutable trade book/ledger, not from the refresh time.
    if mode == "Positional" and not res.empty:
        def _original_trade_time(symbol_value):
            clean = (
                str(symbol_value)
                .replace("★★ ", "")
                .replace("★ ", "")
            ...
            key = _trade_key("NSE", "Positional", clean)
            active_trade = st.session_state.trade_book.get(key)
            if active_trade:
                return active_trade.get("Entry Time", "TIME UNAVAILABLE")

            # Fall back to the persistent ledger if the live session trade book
            # has not yet been restored.
            ledger = trade_report_dataframe(local_now().date())
            if not ledger.empty:
                hit = ledger[
                    (ledger["Module"].astype(str).str.upper() == "NSE")
                    & (ledger["Mode"].astype(str).str.upper() == "POSITIONAL")
                    & (ledger["Symbol"].astype(str).str.upper() == clean.upper())
                ]
                if not hit.empty:
                    return str(hit.iloc[0].get("Entry Time", "TIME UNAVAILABLE"))

            return "TIME UNAVAILABLE"

        res["Entry Date/Time"] = res["Script"].apply(_original_trade_time)
        long_df = res[res["Recommendation"] == "🟢 LONG"].copy()
        short_df = res[res["Recommendation"] == "🔴 SHORT"].copy()
