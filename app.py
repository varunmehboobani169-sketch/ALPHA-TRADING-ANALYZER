...
def normalize_options(df):
    ...
    for names,target in [(["close","ltp","last_price","price"],"close"),(["iv","implied_volatility","impliedvolatility","implied_vol"],"iv"),(["oi","open_interest","openinterest"],"oi"),(["volume","vol","traded_volume"],"volume")]:
        v=num(x,names); r[target]=v if v is not None else float("nan")
...
def normalize_spot(df):
    x=normalize_time(df); p=num(x,["close","ltp","last_price","nifty","spot","index_close","price"])
...
def normalize_vix(df):
    x=normalize_time(df); p=num(x,["close","ltp","last_price","vix_close","vix","price"])
...
def build_features(m):
    ...
    x=m.copy(); x["strike_num"]=pd.to_numeric(x.strike,errors="coerce"); x=x.dropna(subset=["strike_num","nifty_spot"])
    if x.empty: return pd.DataFrame()
    x["expiry_dt"]=pd.to_datetime(x.expiry,errors="coerce"); day=x.timestamp.dt.normalize(); x=x[x.expiry_dt.isna() | (x.expiry_dt.dt.normalize()>=day)].copy()
    if x.empty: return pd.DataFrame()
    nearest=x.groupby("timestamp")["expiry_dt"].transform("min"); x=x[x.expiry_dt.isna() | (x.expiry_dt==nearest)].copy()
    if x.empty: return pd.DataFrame()
    ...
    if not isinstance(wide.columns,pd.MultiIndex): return pd.DataFrame()
    ...
    def wcol(metric,side):
        ...
        if exact in wide.columns: return exact
        for c in wide.columns:
            if metric in str(c) and side in str(c): return c
        return None
    ce_close=wcol("close","CE"); pe_close=wcol("close","PE")
    if ce_close is None or pe_close is None: return pd.DataFrame()
    wide=wide.dropna(subset=[ce_close,pe_close])
    if wide.empty: return pd.DataFrame()
    spot_key=x.groupby(key,as_index=False)["nifty_spot"].first(); wide=wide.merge(spot_key,on=key,how="left"); wide["atm_dist"]=(wide.strike_num-wide.nifty_spot).abs(); atm=wide.sort_values(["timestamp","atm_dist"]).drop_duplicates("timestamp",keep="first")
    if atm.empty: return pd.DataFrame()
    def arr(metric,side):
        c=wcol(metric,side); return atm[c].to_numpy() if c else [float("nan")]*len(atm)
    oi_all=x.pivot_table(index=["timestamp","expiry_dt"],columns="side",values="oi",aggfunc="sum"); atm_idx=pd.MultiIndex.from_frame(atm[["timestamp","expiry_dt"]]); oi_all=oi_all.reindex(atm_idx)
    ce_oi=oi_all["CE"] if "CE" in oi_all.columns else pd.Series(index=atm_idx,dtype=float); pe_oi=oi_all["PE"] if "PE" in oi_all.columns else pd.Series(index=atm_idx,dtype=float); pcr=pe_oi.reset_index(drop=True)/ce_oi.reset_index(drop=True)
    f=pd.DataFrame({"timestamp":atm.timestamp.to_numpy(),"nifty_spot":atm.nifty_spot.to_numpy(),"atm_strike":atm.strike_num.to_numpy(),"ce_close":arr("close","CE"),"pe_close":arr("close","PE"),"ce_iv":arr("iv","CE"),"pe_iv":arr("iv","PE"),"ce_oi":arr("oi","CE"),"pe_oi":arr("oi","PE"),"ce_volume":arr("volume","CE"),"pe_volume":arr("volume","PE"),"pcr_oi":pcr.to_numpy()})
    if "vix_close" in x:
        vix_ts=x.groupby("timestamp").vix_close.last(); f["vix_close"]=pd.to_numeric(vix_ts.reindex(f.timestamp).to_numpy(),errors="coerce")
    else: f["vix_close"]=float("nan")
...
with st.sidebar:
    ...
    access_token=st.text_input("Dhan Access Token",value="",type="password",help="Enter your current Dhan access token. It is kept in the Streamlit session and is not written into GitHub.").strip()
    st.caption(f"Client ID: {client_id or DEFAULT_CLIENT_ID}")
    st.caption("Dhan token is ready for the Dhan-connected module; Q1 pattern analysis below uses uploaded CSV data.")
...
if not opt_files or not spot_files:
    ...
    if st.button("ANALYZE PATTERNS",use_container_width=True):
        ...
        try:
            ...
            o_raw=read_files(opt_files); s_raw=read_files(spot_files); v_raw=read_files(vix_files) if vix_files else pd.DataFrame()
            ...
            status.info("70% — Finding ATM CE/PE pairs"); bar.progress(70,text="FRIDAY: 70% — Finding ATM")
            f=build_features(m)
            if f.empty: raise ValueError("Spot synchronization worked, but no valid CE+PE ATM pair was found.")
            ...
            with zipfile.ZipFile(out,"w",zipfile.ZIP_DEFLATED) as z:
                z.writestr("FRIDAY_features.csv",f.to_csv(index=False)); z.writestr("FRIDAY_patterns.csv",patterns.to_csv(index=False)); z.writestr("FRIDAY_diagnostics.csv",diagnostics(o,s,v,m,f).to_csv(index=False))
