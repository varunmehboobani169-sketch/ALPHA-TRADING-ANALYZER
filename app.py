...
def instrument_candidates(symbol):
    # Explicit Dhan index IDs plus compatibility fallbacks for India VIX.
...
def download_index_ohlc(symbol,start_date,end_date,timeframe,token,client_id):
    ...
    for seg,sid in instrument_candidates(symbol):
        local=[]; failed_reason=None; cur=start_date
        while cur<=end_date:
            ...
            part,reason=dhan_series(body); attempts.append(f"{seg}/{sid} {cur}..{ce}: {reason}")
            if not part.empty: local.append(part)
            cur=ce+timedelta(days=1)
        if local:
            chosen=(seg,sid); chunks=local; break
    if not chunks:
        raise ValueError("No candles returned. Tried: " + " | ".join(attempts[-8:]))
...
def render_data_vault(token,client_id):
    ...
    st.caption("Offline OHLC collector for NIFTY and India VIX. Futures and OI are excluded.")
...
def render_analyzer():
    ...
    opt=st.file_uploader("Q1 Option Data (CSV)",type=["csv"],accept_multiple_files=True); spot=st.file_uploader("Q1 NIFTY Spot Data (CSV)",type=["csv"],accept_multiple_files=True); vix=st.file_uploader("Q1 India VIX Data (CSV)",type=["csv"],accept_multiple_files=True)
    if not opt or not spot: st.info("Upload the Q1 Option and NIFTY Spot CSV files. India VIX is optional."); return
...
with st.sidebar:
    ...
    if token: st.success("Dhan token entered for this session")
