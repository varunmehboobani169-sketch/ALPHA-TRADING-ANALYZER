...
with st.sidebar:
    ...
    expiry_code = st.selectbox("Expiry code", [0, 1, 2], index=0, help="0 = near, 1 = next, 2 = farther expiry", key="history_expiry_code")
...
def rolling(index_name: str, level: int, side: str, start, end):
    ...
    rows = []
    # The API expects CALL / PUT, while the dashboard uses CE / PE labels.
...
def safe_df(rows: list[dict]) -> pd.DataFrame:
    if not rows:
        return pd.DataFrame(columns=["date", "index", "level", "side", "strike", "spot", "iv", "oi", "close"])
...
def render_results(data: pd.DataFrame, errors: list[str], fetched_at: datetime):
    ...
    oi["OI Change vs Previous Observation"] = oi.sort_values("date").groupby(["index", "level", "side"])["OI"].diff()
    st.dataframe(oi[["date", "index", "level", "strike", "side", "OI", "OI Change vs Previous Observation"]], use_container_width=True, hide_index=True)
    ...
    st.dataframe(iv[["date", "index", "level", "strike", "side", "spot", "IV", "IV Change vs Prior Day", "oi"]], use_container_width=True, hide_index=True)
    ...
    if auto_refresh:
        try:
            ...
            if not fresh_data.empty:
                ...
                st.session_state[TIME_KEY] = datetime.now(IST)
                render_results(fresh_data, fresh_errors, st.session_state[TIME_KEY])
        except Exception as exc:
