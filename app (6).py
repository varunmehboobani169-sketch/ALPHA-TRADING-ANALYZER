...
def mcx_running_positional_trade(h, box_pct=0.0025):
    """
    Identify the currently running daily P&F positional direction and derive
    a practical entry + structural SL from the most recent 3-box reversal.

    X latest column => existing LONG position
      Entry = reversal level that started the current X column
      SL    = previous O-column low

    O latest column => existing SHORT position
      Entry = reversal level that started the current O column
      SL    = previous X-column high

    If fewer than 2 P&F columns exist, no running position is reported.
    """
    cols = build_pnf(h["close"], box_pct, 3)
    if len(cols) < 2:
        return {
            "position": None,
            "entry": np.nan,
            "sl": np.nan,
            "reason": "Not enough P&F columns",
        }
    ...
    if current["type"] == "X" and previous["type"] == "O":
        # Bullish 3-box reversal from the prior O column.
        entry = previous["low"] * ((1 + box_pct) ** reversal)
        sl = previous["low"]
        return {
            "position": "LONG",
            "entry": float(entry),
            "sl": float(sl),
            "reason": "Current daily P&F column is X; bullish position is running",
        }

    if current["type"] == "O" and previous["type"] == "X":
        # Bearish 3-box reversal from the prior X column.
        entry = previous["high"] * ((1 - box_pct) ** reversal)
        sl = previous["high"]
        return {
            "position": "SHORT",
            "entry": float(entry),
            "sl": float(sl),
            "reason": "Current daily P&F column is O; bearish position is running",
        }

    return {
        "position": "LONG" if current["type"] == "X" else "SHORT",
        "entry": np.nan,
        "sl": float(previous["low"] if current["type"] == "X" else previous["high"]),
        "reason": "Current P&F direction is active; entry level unavailable from current column structure",
    }
...
if page == "Market Overview":
    ...
    if mode == "Intraday":
        ...
        st.caption(
            "MCX Positional: show only currently running daily P&F positions. "
            "X column = LONG, O column = SHORT. Entry is the latest 3-box reversal level; SL is the previous opposite column extreme."
        )
    ...
    for i, (_, r) in enumerate(candidates.iterrows(), 1):
        ...
        try:
            ...
            if mode == "Intraday":
                ...
                # Positional MCX shows ONLY currently running P&F positions.
                running = mcx_running_positional_trade(h, 0.0025)
                if running["position"] == "LONG":
                    rec = "🟢 LONG"
                    bias = "Bullish"
                elif running["position"] == "SHORT":
                    rec = "🔴 SHORT"
                    bias = "Bearish"
                else:
                    rec = "NO POSITION"
            ...
            if new_alert and alerts_enabled:
                trigger_sound_alert(f"MCX {mode}", symbol, rec)
